...
/** Eenduidig lettertype/grootte voor alle schaal-intro’s ("Schaal 1–7: ...") in de vragenlijst. */
const SCALE_INTRO_CLASS = "text-sm text-muted-foreground";
...
/** Likert-schaal 1–7: balk met schuifje van links (1) naar rechts (7), daaronder cijfers; optioneel labels links/rechts boven de balk */
function ScaleSlider({
  ...
  const showLabels = labelLeft != null || labelRight != null;
  return (
...
const Q3_OPTIONS: { id: string; label: string; tooltip?: string }[] = [
  { id: "Uitbreiden", label: "Uitbreiden", tooltip: "Meer koeien per hectare of meer koeien op het bedrijf" },
  { id: "Ongeveer gelijk blijven", label: "Ongeveer gelijk blijven" },
  { id: "Extensiveren", label: "Extensiveren", tooltip: "Minder koeien per hectare" },
  { id: "Stoppen zonder opvolger", label: "Stoppen zonder opvolger" },
  { id: "Overdragen", label: "Overdragen" },
  { id: "Nog geen duidelijke richting", label: "Nog geen duidelijke richting" },
];
...
const Q11_THEMES: { key: keyof UniveFormData; keyToelichting: keyof UniveFormData; label: string }[] = [
  {
    ...
    label:
      "Duurzaamheidsmaatregelen (Denk aan maatregelen rond biodiversiteit, emissiereductie, bodemverbetering en natuurbeheer.)",
  },
  {
    key: "q11_bedrijfsschaal",
    keyToelichting: "q11_bedrijfsschaal_toelichting",
    label:
      "Bedrijfsschaal (Bijvoorbeeld uitbreiding of juist extensivering van de veestapel of het grondgebruik.)",
  },
  {
    key: "q11_bedrijfsvoering",
    keyToelichting: "q11_bedrijfsvoering_toelichting",
    label:
      "Bedrijfsvoering (Zoals andere voersystemen, meer of minder beweiding of aanpassingen in het dagelijkse management.)",
  },
  {
    key: "q11_verdienmodel",
    keyToelichting: "q11_verdienmodel_toelichting",
    label:
      "Verdienmodel / verbreding (Denk aan verkoop van eigen producten, energieopwekking of recreatie op en rond het bedrijf.)",
  },
  {
    key: "q11_investeringen",
    keyToelichting: "q11_investeringen_toelichting",
    label:
      "Investeringen in gebouwen, stal of techniek (Bijvoorbeeld nieuwbouw, verbouw of technische aanpassingen in de stal.)",
  },
];
...
/** Splitst de optie-tekst in hoofdlabel + laatste stuk tussen haakjes (voor tooltip). */
function splitQ16LabelAndTooltip(option: string): { label: string; tooltip?: string } {
  const trimmed = option.trim();
  if (!trimmed.endsWith(")")) return { label: option };
  const idx = trimmed.lastIndexOf(" (");
  if (idx === -1) return { label: option };
  const label = trimmed.slice(0, idx).trim();
  const inner = trimmed.slice(idx + 2, -1).trim(); // inhoud binnen laatste haakjes
  if (!inner) return { label: option };
  return { label, tooltip: inner };
}
...
export function buildUniveScreens(): UniveScreen[] {
    ...
    render: (fd, update) => (
      <div className="space-y-2">
        <Input
          ...
          maxLength={100}
          className="w-full"
          placeholder={'Bijvoorbeeld "Súdwest-Fryslân"'}
        />
      </div>
    ),
  });

  // Vraag 3 – type bedrijf (URL q3)
  screens.push({
    id: "q3",
    ...
    render: (fd, update, toggleMulti) => {
      const q1Tooltips: Record<string, string> = {
        Gangbaar:
          "Een regulier melkveebedrijf dat niet biologisch gecertificeerd is en volgens de gangbare landbouwpraktijk werkt.",
        Natuurinclusief:
          "Een bedrijfsvoering waarbij actief rekening wordt gehouden met natuur, biodiversiteit, bodem en landschap, bijvoorbeeld door kruidenrijk grasland, landschapselementen of extensiever beheer.",
        Biologisch: "Een bedrijf dat gecertificeerd biologisch produceert.",
        "In omschakeling":
          "Je bent momenteel bezig met de overgang naar een andere bedrijfsvoering.",
      };
      return (
        <>
          <RadioGroup
  ...
  // Vraag 4 – grootte bedrijf (URL q4)
  screens.push({
    id: "q4",
    group: "Deel 1 – Je bedrijf",
    stepNumber: 0,
  ...
  // Vraag 5 – ontwikkeling bedrijf (URL q5)
  screens.push({
    id: "q5",
    group: "Deel 1 – Je bedrijf",
    stepNumber: 0,
      ...
      <RadioGroup value={fd.q3 ?? ""} onValueChange={(v) => update({ q3: v })} className="space-y-2">
        {Q3_OPTIONS.map((opt) => (
          <div key={opt.id} className={OPTION_ROW_CLASS}>
            ...
            <Label htmlFor={`q3-${opt.id}`} className="flex-1 cursor-pointer text-sm font-medium">
              {opt.tooltip ? (
                <TermWithTooltip term={opt.label} explanation={opt.tooltip} />
              ) : (
                opt.label
              )}
            </Label>
  ...
  // q4a: doorvraag regelgeving (geen eigen URL, inlined op q6)
  screens.push({
    id: "q4a",
    group: "Deel 2 – Huidige situatie en toekomstbeeld",
    stepNumber: 0,
    ...
    render: (fd, update, _toggleMulti) => (
      <div className="space-y-6">
        <p className={SCALE_INTRO_CLASS}>
          Schaal 1–7: links <span className="font-semibold text-foreground">helemaal oneens</span>, rechts{" "}
          <span className="font-semibold text-foreground">helemaal eens</span>.
        </p>

        <div className="space-y-2">
          <Label className={QUESTION_LABEL_CLASS}>
            Verduurzaming is belangrijk voor de continuïteit van mijn bedrijf.{" "}
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
          ...
          <ScaleSlider value={fd.q5a ?? 4} onValueChange={(v) => update({ q5a: v })} />
        </div>

        <div className="space-y-2">
          <Label className={QUESTION_LABEL_CLASS}>
            CO₂-reductie is de komende 10 jaar belangrijk voor mijn bedrijf.{" "}
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
          ...
          <ScaleSlider value={fd.q5b ?? 4} onValueChange={(v) => update({ q5b: v })} />
        </div>

        <div className="space-y-2">
          <Label className={QUESTION_LABEL_CLASS}>
            Biodiversiteit is belangrijk voor mijn bedrijf.{" "}
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
          ...
          <ScaleSlider value={fd.q5c ?? 4} onValueChange={(v) => update({ q5c: v })} />
        </div>

        <div className="rounded-lg border border-border bg-muted/30 p-3">
          <Label className="mb-2 block text-sm font-medium text-foreground">
            Wil je je antwoorden kort toelichten?
          </Label>
          <Textarea
            value={fd.q5_toelichting ?? ""}
            onChange={(e) => update({ q5_toelichting: e.target.value })}
  ...
  const Q14_OPEN_SLIDER_OPTIONS: { key: keyof UniveFormData; label: string }[] = [
    ...
    { key: "q14_open_teeltverzekering", label: "Teeltverzekering" },
  ];
  screens.push({
    id: "q14",
    ...
    render: (fd, update, _toggleMulti) => (
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2" role="group">
            <Label className="text-sm font-medium text-foreground">Andere ondersteuning, namelijk</Label>
            <Input
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-3">
          ...
          <Textarea
            ...
            onChange={(e) => update({ q14_open_toelichting: e.target.value })}
            placeholder="Schrijf hier kort je toelichting"
            rows={3}
            maxLength={500}
          />
  ...
  // Vraag 15 – welke vormen ondersteuning waardevol (optioneel max 2, of Geen)
  ...
  screens.push({
    ...
    render: (fd, update, toggleMulti) => {
      ...
      const onlyGeen = selected.length === 1 && selected[0] === "Geen";
      const hasGeen = selected.includes("Geen");
      const onToggle = (opt: string) => {
        if (opt === "Geen") {
          update({ q15_waardevol: hasGeen ? [] : ["Geen"], q15_waardevol_anders: hasGeen ? fd.q15_waardevol_anders : "", q15_waardevol_toelichting: hasGeen ? fd.q15_waardevol_toelichting : "" });
          return;
        }
        if (hasGeen) {
          const rest = selected.filter((x) => x !== "Geen");
          const next = rest.includes(opt) ? rest.filter((x) => x !== opt) : [...rest, opt].slice(0, 2);
          update({ q15_waardevol: next });
          return;
        }
        toggleMulti("q15_waardevol", opt, 2);
      };
      return (
        <div className="space-y-4">
          <p className={HELPER_TEXT_CLASS}>
            Kies maximaal 2 in volgorde van voorkeur, of Geen. ({selected.filter((x) => x !== "Geen").length}/2 gekozen)
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {Q15_WAARDEVOL_OPTIONS.map((opt) => {
              const isSelected = selected.includes(opt);
              const rank = isSelected && opt !== "Geen" ? selected.filter((x) => x !== "Geen").indexOf(opt) + 1 : 0;
              const disabled = opt !== "Geen" && hasGeen && !isSelected;
              const disabledMax = opt !== "Geen" && !hasGeen && selected.length >= 2 && !isSelected;
              return (
                <button
                  key={opt}
                  type="button"
                  onClick={() => !disabled && !disabledMax && onToggle(opt)}
                  ...
                  className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm transition-all ${
                    isSelected
                      ? "border-primary bg-primary/10 text-primary"
                      : "border-border bg-card hover:border-primary/50"
                  } ${disabled || disabledMax ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  {isSelected && opt !== "Geen" && (
                    <span
          ...
          {!onlyGeen && selected.length > 0 && (
            <div className="rounded-lg border border-border bg-muted/30 p-3">
              <Label className="mb-2 block text-sm font-medium text-foreground">Licht je antwoorden kort toe</Label>
              <Textarea
                value={fd.q15_waardevol_toelichting ?? ""}
                onChange={(e) => update({ q15_waardevol_toelichting: e.target.value })}
                placeholder="Schrijf hier kort je toelichting"
                rows={3}
                maxLength={500}
              />
  ...
  screens.push({
    ...
    required: true,
    render: (fd, update, toggleMulti) => {
      const selectedA = Array.isArray(fd.q16a) ? fd.q16a : [];
      const selectedB = Array.isArray(fd.q16b) ? fd.q16b : [];
      const hasGeenAllenA = selectedA.includes("Geen van allen");
      const hasGeenAllenB = selectedB.includes("Geen van allen");
      return (
        <div className="space-y-8">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {Q16A_OPTIONS.map((opt) => {
                const { label, tooltip } = splitQ16LabelAndTooltip(opt);
                const isSelected = selectedA.includes(opt);
                const isGeenAllen = opt === "Geen van allen";
                const disabled = !isSelected && hasGeenAllenA && !isGeenAllen;

                const handleClick = () => {
                  if (disabled) return;
                  if (isGeenAllen) {
                    // "Geen van allen" overschrijft eerder gekozen opties in deze vraag
                    update({ q16a: isSelected ? [] : ["Geen van allen"] });
                  } else {
                    const current = selectedA.filter((x) => x !== "Geen van allen");
            ...
            {selectedA.includes("Anders, namelijk:") && (
              <div className="mt-3">
                <Label className={`${QUESTION_LABEL_CLASS} after:content-['*'] after:text-destructive`}>
                  Anders, namelijk: (verplicht)
                </Label>
          ...
          <div className="space-y-4 border-t border-border pt-6">
            <h3 className="text-base font-semibold text-foreground">
              Stel dat deze activiteiten financieel rendabel zijn, welke zou je dan het liefst toepassen?
            </h3>
            <p className={HELPER_TEXT_CLASS}>
              Geef een top 3 door in volgorde van voorkeur, of kies &quot;Geen van allen&quot;. ({selectedB.includes("Geen van allen") ? 0 : selectedB.length}/3 gekozen)
            </p>
            <div className="flex flex-wrap gap-2">
              {Q16A_OPTIONS.map((opt) => {
                const { label, tooltip } = splitQ16LabelAndTooltip(opt);
                const isSelected = selectedB.includes(opt);
                const isGeenAllen = opt === "Geen van allen";
                const rank = isSelected && !isGeenAllen ? selectedB.filter((x) => x !== "Geen van allen").indexOf(opt) + 1 : 0;
                const maxReached = !isSelected && selectedB.filter((x) => x !== "Geen van allen").length >= 3;
                const disabled =
                  (!isSelected && hasGeenAllenB && !isGeenAllen) ||
                  (!isSelected && isGeenAllen && selectedB.filter((x) => x !== "Geen van allen").length > 0) ||
                  (!isSelected && !isGeenAllen && maxReached);
                const handleClick = () => {
                  if (disabled) return;
                  if (isGeenAllen) {
                    // Kiezen voor "Geen van allen" wist andere keuzes en zet alleen deze aan
                    update({ q16b: isSelected ? [] : ["Geen van allen"] });
                  } else {
                    // Normale top-3 logica zonder "Geen van allen"
                    const current = selectedB.filter((x) => x !== "Geen van allen");
                    const exists = current.includes(opt);
                    const next = exists ? current.filter((x) => x !== opt) : [...current, opt].slice(0, 3);
                    update({ q16b: next });
                  }
                };
                return (
            ...
            {selectedB.includes("Anders, namelijk:") && (
              <div className="mt-3">
                <Label className={`${QUESTION_LABEL_CLASS} after:content-['*'] after:text-destructive`}>
                  Anders, namelijk: (verplicht)
                </Label>
  ...
  const Q16_SLIDER_OPTIONS: { key: keyof UniveFormData; label: string; tooltip?: string }[] = [
    {
      key: "q16_marge",
      label: "Voldoende inkomen uit het bedrijf",
      tooltip: "Voldoende inkomen om van te leven en het bedrijf gezond te houden.",
    },
    {
      key: "q16_stabiliteit",
      label: "Stabiel inkomen met beperkte schommelingen",
    },
    {
    ...
    },
    {
      ...
      label: "Continuïteit van het bedrijf voor de volgende generatie",
      tooltip: "Het bedrijf in zo’n staat achterslaten dat een opvolger goed kan doorgaan.",
    },
    {
      key: "q16_voortzetten",
    ...
    },
    {
      ...
      tooltip: "Dat de waarde van grond, gebouwen en bedrijf behouden blijft en op termijn niet sterk daalt.",
    },
    {
      key: "q16_werkplezier",
      label: "Werkplezier en voldoening uit het bedrijf",
    },
  ];
  screens.push({
    id: "q16",
    ...
    render: (fd, update, _toggleMulti) => (
      <div className="space-y-5">
        <p className={SCALE_INTRO_CLASS}>
          Schaal 1–7: links <span className="font-semibold text-foreground">niet belangrijk</span>, rechts{" "}
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-3">
          ...
          <Textarea
            ...
            onChange={(e) => update({ q16_toelichting: e.target.value })}
            placeholder="Schrijf hier kort je toelichting"
            rows={3}
            maxLength={500}
          />
  ...
  screens.push({
    ...
    render: (fd, update, _toggleMulti) => (
      <div className="rounded-lg border border-border bg-muted/30 p-4">
        <Textarea
          value={fd.q19a ?? ""}
          onChange={(e) => update({ q19a: e.target.value })}
          placeholder="Schrijf hier kort je toelichting"
          rows={5}
          maxLength={1500}
          className="w-full"
          ...
          >
            ...
            {toestemming === "Ja" && (
              <div className="space-y-3 pl-9">
                <p className={HELPER_TEXT_CLASS}>
                  Let op: je deelname is dan <span className="font-semibold underline">niet meer anoniem</span>. We koppelen je gegevens aan jouw antwoorden.
                </p>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div>
                    <Label htmlFor="q19_naam" className="mb-1 block text-sm font-medium text-foreground">Naam</Label>
                    <Input
                      id="q19_naam"
                      value={fd.q19_naam ?? ""}
                  ...
                  <div>
                    <Label htmlFor="q19_email" className="mb-1 block text-sm font-medium text-foreground">E-mailadres</Label>
                    <Input
                      id="q19_email"
                      type="email"
                      value={fd.q19_email ?? ""}
                      onChange={(e) => {
                        ...
                        update({
                          q19_email: v,
                          ...(fd.q19_toestemming_contact === "Ja" && fd.q21_verloting === "Ja" ? { q21_email: v } : {}),
                        });
                      }}
                  ...
                  <div>
                    <Label htmlFor="q19_telefoon" className="mb-1 block text-sm font-medium text-foreground">Telefoonnummer</Label>
                    <Input
                      id="q19_telefoon"
          ...
          <div className="border-t border-border pt-4">
            <div className="flex items-start gap-3">
              <input
                type="checkbox"
                id="q21_verloting"
                checked={verlotingChecked}
                onChange={(e) => {
                  const checked = e.target.checked;
                  update({
                    q21_verloting: checked ? "Ja" : "Nee",
                    q21_email: checked ? (toestemming === "Ja" ? (fd.q19_email ?? "") : (fd.q21_email ?? "")) : "",
                  });
                }}
                className="mt-1 h-4 w-4 shrink-0 rounded border-border accent-primary"
                aria-label="Meedoen aan verloting"
              />
              <Label htmlFor="q21_verloting" className="cursor-pointer text-sm font-medium text-foreground">
                Wil je meedoen aan de verloting van een boerenpakket onder de deelnemers aan deze vragenlijst?
              </Label>
            </div>
            {verlotingChecked && toestemming === "Ja" && (
              <p className="mt-2 text-xs text-muted-foreground">
                We gebruiken het e-mailadres dat je hierboven hebt opgegeven voor de verloting.
              </p>
            )}
            {verlotingChecked && toestemming === "Nee" && (
              <div className="mt-3 space-y-2 rounded-lg border border-border bg-muted/30 p-3">
                <Label htmlFor="q21_email" className="mb-1 block text-sm font-medium text-foreground">
                  E-mailadres
                </Label>
                <Input
                  id="q21_email"
                ...
                <p className="text-xs text-muted-foreground">
                  Je antwoorden blijven anoniem. Dit e-mailadres wordt alleen voor de verloting gebruikt en niet gekoppeld aan je vragenlijst.
                </p>
              </div>
            )}
          </div>
  ...
  for (let i = 0; i < screens.length; i++) {
    if (screens[i].id === "q4a") {
      screens[i].stepNumber = 6; //zelfde stap als q6 (doorvraag inlined)
      continue;
    }
...
export function isUniveStepValid(screen: UniveScreen, fd: UniveFormData): boolean {
  ...
  if (screen.id === "q10") {
    ...
    for (const { key, keyToelichting } of themes) {
      ...
      if (val === "Ja") {
        const toelichting = ((fd[keyToelichting] as string) ?? "").trim();
        if (!toelichting) return false; // minimaal 1 teken verplicht
      }
    }
    return true;
  }
  if (screen.id === "q11b") {
    ...
    const toelichtingZichtbaar = !q11b.includes("Nieuwe regelgeving") && !q11b.includes("Anders, namelijk:");
    if (toelichtingZichtbaar) {
      const toel = typeof fd.q11b_toelichting === "string" ? fd.q11b_toelichting.trim() : "";
  ...
  if (screen.id === "q8") {
    const val = typeof fd.q6 === "string" ? fd.q6.trim() : "";
    return val.length >= 1;
  }
  if (screen.id === "q12") {
    const val = typeof fd.q10 === "string" ? fd.q10.trim() : "";
    return val.length >= 1;
  }
  if (screen.id === "q6") {
  ...
  if (screen.id === "q13") {
    ...
    {
      const toel = typeof fd.q11_toelichting === "string" ? fd.q11_toelichting.trim() : "";
      if (!toel) return false;
    }
  }
  if (screen.id === "q15") {
    const q15w = Array.isArray(fd.q15_waardevol) ? fd.q15_waardevol : [];
    if (q15w.length === 0) return true;
    const onlyGeen = q15w.length === 1 && q15w[0] === "Geen";
    if (onlyGeen) return true;
    if (q15w.includes("Anders, namelijk:")) {
      const anders = typeof fd.q15_waardevol_anders === "string" ? fd.q15_waardevol_anders.trim() : "";
      if (!anders) return false;
    }
    const toel = typeof fd.q15_waardevol_toelichting === "string" ? fd.q15_waardevol_toelichting.trim() : "";
    if (!toel) return false;
    return true;
  }
  if (screen.id === "q16a") {
    ...
    const hasGeenAllen = q16b.includes("Geen van allen");
    if (!hasGeenAllen && q16b.length !== 3) return false;
    if (hasGeenAllen && q16b.length !== 1) return false;
    if (q16b.includes("Anders, namelijk:")) {
      const andersB = typeof fd.q16b_anders === "string" ? fd.q16b_anders.trim() : "";
      if (!andersB) return false;
    }
    return true;
  }
  if (screen.id === "q16") {
    const toel = typeof fd.q16_toelichting === "string" ? fd.q16_toelichting.trim() : "";
    if (!toel) return false;
  }
  if (screen.id === "q7") {
    const toel = typeof fd.q5_toelichting === "string" ? fd.q5_toelichting.trim() : "";
    if (!toel) return false;
  }
  if (screen.id === "q9") {
    const toel = typeof fd.q10_toelichting === "string" ? fd.q10_toelichting.trim() : "";
    if (!toel) return false;
  }
  if (screen.id === "q14") {
    const toel = typeof fd.q14_open_toelichting === "string" ? fd.q14_open_toelichting.trim() : "";
    if (!toel) return false;
  }
  if (screen.id === "q18") {
    const toel = typeof fd.q17_toelichting === "string" ? fd.q17_toelichting.trim() : "";
    if (!toel) return false;
  }
  if (screen.id === "q20") {
    const v19a = typeof fd.q19a === "string" ? fd.q19a.trim() : "";
    if (!v19a) return false;
  }
...
/** Of dit scherm alleen getoond wordt onder een voorwaarde (q4a, q11b). q4a is altijd verborgen: inhoud staat inlined op q6. */
